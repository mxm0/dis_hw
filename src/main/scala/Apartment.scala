...
class Apartment {
  ...
  /*
    ...
    floors varchar(255), price varchar(40), garden varchar(40), rooms integer,
    balcony smallint, builtinKitchen smallint, estate_id int,
    constraint appartment_constraint foreign key (estate_id) references estate(id) on delete cascade)
   */
    ...
    try {
      if (_id == -1) {
        ...
        val insertSQL =
          """
            |INSERT INTO
            |appartment(floors, garden, price, estate_id)
            |VALUES (?, ?, ?, ?)
          """
